...
<header class="site-header">
  ...
  <nav><a href="#about">회사소개</a><a href="#business">사업분야</a><a href="#products">제품소개</a><a href="#finder">도료찾기</a><a href="#makers">제조사 자료</a><a href="#channels">소식</a><a href="#contact">문의하기</a></nav>
</header>
<main>
  <section id="home" class="hero">
    <img class="hero-image" src="/plb-warehouse-main.jpg" alt="PLB 산업용 도료 창고">
  ...
  <section id="makers" class="section makers-section"><p class="eyebrow dark">OFFICIAL RESOURCES</p><h2>제조사 공식자료</h2><div class="maker-grid"><a href="https://www.kccworld.co.kr/" target="_blank" rel="noopener"><strong>KCC</strong><span>공식 홈페이지 →</span></a><a href="https://samhwa.com/" target="_blank" rel="noopener"><strong>삼화페인트</strong><span>공식 홈페이지 →</span></a><a href="https://www.ckpc.co.kr/" target="_blank" rel="noopener"><strong>조광페인트</strong><span>공식 홈페이지 →</span></a><a href="https://www.jevisco.com/" target="_blank" rel="noopener"><strong>제비스코</strong><span>공식 홈페이지 →</span></a></div></section>
  <section class="section two-column sponsor-section"><figure class="sponsor-photo"><img src="/plb-sponsor.jpg" alt="PLB 드라마 협찬"></figure><div><p class="eyebrow dark">DRAMA SPONSORSHIP</p><h2>콘텐츠 속에서도 만나는 PLB</h2><p>드라마 협찬을 통해 산업용 도료 전문기업의 브랜드와 신뢰를 더 넓게 알리고 있습니다.</p><a class="primary" href="https://blog.naver.com/plb6498" target="_blank" rel="noopener">협찬 소식 보기</a></div></section>
...
function renderProducts() {
  ...
  document.querySelectorAll('.product-inquiry').forEach((button) => button.addEventListener('click', () => { document.querySelector('#subject').value = `${button.dataset.product} 문의`; document.querySelector('#message').value = `${button.dataset.product}의 용도, 규격, 재고 및 견적 상담을 요청합니다.`; document.querySelector('#contact').scrollIntoView({ behavior: 'smooth' }); }));
}
...
function updateFinder() { document.querySelector('#finderTitle').textContent = `${material.value}용 ${fn.value} 도료 상담`; document.querySelector('#finderDescription').textContent = `${material.value} · ${fn.value} · ${environment.value} 조건으로 제품 및 제조사 상담을 준비합니다.`; document.querySelector('#subject').value = `${material.value}용 ${fn.value} 도료`; document.querySelector('#message').value = `사용 소재: ${material.value}\n필요 기능: ${fn.value}\n사용 환경: ${environment.value}\n\n추천 제품 및 견적 상담을 요청합니다.`; document.querySelector('#contact').scrollIntoView({ behavior: 'smooth' }); }
document.querySelector('#applyFinder').addEventListener('click', updateFinder);
document.querySelector('#inquiryForm').addEventListener('submit', async (event) => { event.preventDefault(); const status = document.querySelector('#formStatus'); const data = Object.fromEntries(new FormData(event.currentTarget)); status.textContent = '문의 내용을 저장하고 있습니다.'; if (!supabase) { status.textContent = '온라인 접수 연결 전입니다. 전화 055-313-6778 또는 이메일로 문의해주세요.'; return; } const { error } = await supabase.from('inquiries').insert(data); if (error) { status.textContent = '문의 저장에 실패했습니다. 전화 또는 이메일로 연락해주세요.'; return; } event.currentTarget.reset(); status.textContent = '문의가 정상적으로 접수되었습니다.'; });